...
async function checkPutPayload(req, res, next) {
  try {
    const { vin, make, model, mileage } = req.body;

    const oldVin = await db("cars").where({ id: req.params.id }).first();
    ...
    if (!vin || !make || !model || !mileage) {
      ...
      if (isValid && !searchForVin) {
        next();
      } 
      else if (!isValid) {
        next({ status: 400, message: `vin ${vin} is invalid` })
      } 
      else if (searchForVin) {
        next({status : 400, message : `vin ${vin} already exists`})
      }
    }
  } catch (err) {
